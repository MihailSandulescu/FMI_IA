...
class Joc:
    ...
    def final(self, jucator):
        ...
        for line in self.matr:
            scor_max += line.count(self.JMAX)
            scor_min += line.count(self.JMIN)

        if scor_max > scor_min:
            return self.JMAX
        ...
        for i in range(8):
            for j in range(8):
                if self.matr[i][j] == jucator:
                    for mutare in moves:
                        succesor = copy.deepcopy(self.matr)
                        succesor[i+mutare[0]][j+mutare[1]] = jucator
                        l_mutari.append(Joc(succesor))
