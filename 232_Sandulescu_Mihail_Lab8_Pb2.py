...
class Joc:
    ...
    def final(self):
        ...
        rez = False
        board = self.matr
        max_line_win = self.JMAX * self.NR_CONNECT
        min_line_win = self.JMIN * self.NR_CONNECT


        # verificam linii
        for ndx_line in range(self.NR_LINII):
            for ndx_col in range(self.NR_COLOANE + 1 - self.NR_CONNECT):
                if board[ndx_line][ndx_col : ndx_col + self.NR_CONNECT] == list(max_line_win):
                    return self.JMAX
                elif board[ndx_line][ndx_col : ndx_col + self.NR_CONNECT] == list(min_line_win):
                    return self.JMIN

        # verificam coloane
        for ndx_line in range(self.NR_LINII + 1 - self.NR_CONNECT):
            for ndx_col in range(self.NR_COLOANE):
                if board[ndx_col][ndx_line : ndx_line + self.NR_CONNECT][ndx_col] == list(max_line_win):
                    return self.JMAX
                elif board[ndx_col][ndx_line : ndx_line + self.NR_CONNECT][ndx_col] == list(min_line_win):
                    return self.JMIN
        ...
        for ndx_line in range(self.NR_LINII + 1 - self.NR_CONNECT):
            for ndx_col in range(self.NR_COLOANE + 1 - self.NR_CONNECT):
                for step in range(self.NR_CONNECT):
                    if board[ndx_line : ndx_line + step][ndx_col : ndx_col + step][step] == list(max_line_win):
                        return self.JMAX
                    elif board[ndx_line : ndx_line + step][ndx_col : ndx_col + step][step] == list(min_line_win):
                        return self.JMIN


        # verificam diagonale /
        for ndx_line in range(self.NR_LINII + 1 - self.NR_CONNECT):
            for ndx_col in reversed(range(self.NR_COLOANE - self.NR_CONNECT, self.NR_COLOANE)):
                for step in range(self.NR_CONNECT):
                    if board[ndx_line : ndx_line + step][ndx_col : ndx_col - step][step] == list(max_line_win):
                        return self.JMAX
                    elif board[ndx_line : ndx_line + step][ndx_col : ndx_col - step][step] == list(min_line_win):
        ...
        all_poz = [self.matr[i][j] for i in range(self.NR_LINII) for j in range(self.NR_COLOANE)]

        if rez==False  and  Joc.GOL not in all_poz:
            return 'remiza'
        else:
            return False
    ...
    def mutari(self, jucator_opus):
        ...
        for ndx_col in range(self.NR_COLOANE):
            #daca coloana este plina
            if self.matr[0][ndx_col] != Joc.GOL:
                continue
            else:
                ndx_l = 0
                while self.matr[ndx_l][ndx_col] == Joc.GOL:
                    ndx_l = ndx_l + 1
                board_c = copy.deepcopy(self.matr)
                board_c[ndx_l][ndx_col] = jucator_opus
                l_mutari.append(Joc(board_c))
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        rez = 0
        empty_line = Joc.GOL * self.NR_CONNECT

        # linii
        for ndx_l in range(self.NR_LINII):
            for ndx_c in range(self.NR_COLOANE + 1 - self.NR_CONNECT):
                if self.matr[ndx_l][ndx_c : ndx_c + self.NR_CONNECT] == list(empty_line):
                    rez += 1

        # coloane
        for ndx_l in range(self.NR_LINII + 1 - self.NR_CONNECT):
            for ndx_c in range(self.NR_COLOANE):
                if self.matr[ndx_c][ndx_l : ndx_l + self.NR_CONNECT][ndx_c] == list(empty_line):
                    rez += 1
        ...
        for ndx_l in range(self.NR_LINII):
            for ndx_c in range(self.NR_COLOANE + 1 - self.NR_CONNECT):
                for step in range(self.NR_CONNECT):
                    if self.matr[ndx_l : ndx_l + step][ndx_c : ndx_c + step][step] == list(empty_line):
                        rez += 1
