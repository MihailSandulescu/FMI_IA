# o configuratie este un nod al grafului. o muchie reprezinta o mutare de la o configuratie la alta.
# ponderea tuturor mutarilor este 1, deoarece mutam un bloc de pe o stiva pe alta
class Configuratie:
    ...
    def pozitie(self):
        pozitie = {}
        #enumerate ofera si un counter la iterare (in acest caz index_stiva si index_bloc)
    ...
    def expandeaza(self):
        ...
        for crt_stiva in range(nr_stive):
            for stiva_scop in range(nr_stive):
                # daca nu exista blocuri in stiva curenta, continue (urmarim gasirea unui bloc de mutat)
                if not configuratie.stive[crt_stiva]:
                    continue
                
                # blocul din varful stivei
                de_mutat = configuratie.stive[crt_stiva][len(configuratie.stive[crt_stiva]) - 1]

                cfg_noua = []
                for i in range(nr_stive):
                    if i == stiva_scop:
                        #adaugam blocul mutat la stiva scop
                        stiva_noua = configuratie.stive[i] + [de_mutat]
                    elif i == crt_stiva:
                        # doar blocurile de sub cel mutat
                        stiva_noua = configuratie.stive[i][ : len(configuratie.stive[i]) - 1]
                    else:
                        stiva_noua = configuratie.stive[i]
                    cfg_noua.append(stiva_noua)

                configuratie_noua = Configuratie(cfg_noua)

                # am mai facut inainte aceasta mutare? verificam
                succesor = problema.cauta_nod_nume(configuratie_noua)

                # daca nu, adaugam in arborele de cautare noul nod
                if not succesor:
